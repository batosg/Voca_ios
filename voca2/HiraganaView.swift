...
struct hiraganaView: View {
    // ContentViewの変数をバインディングすることによって複数のViewで変数を使えるようになる
            ...
            VStack {
                ...
                HStack {
                    VStack {
                        ...
                        // 文字盤切替ボタン
                        Button(action:{
                            screen = "dakuHira"  // 50音文字盤（(半)濁音ひらがな）への遷移
                        }) {
                            Text("かな/カナ")
                                .font(.system(size: UIScreen.main.bounds.width * 0.025, weight: .black))
                                .foregroundColor(Color.white)
                    ...
                    Group {
                        VStack {
                            ...
                            ForEach(0..<5) { index in
                                Button(action: {
                                    theText += "\(scan.hiraganaSet[index])"
                                    // 読み上げ
                                    let utterance = AVSpeechUtterance(string: scan.hiraganaSet[index])
                                    utterance.voice = AVSpeechSynthesisVoice(language: "ja-JP")
                                    utterance.rate = 0.5
                                    utterance.volume = playvol
                ...
                if (scan.selected == "aiueo") {
                    ...
                    if (scan.selected == "kirikaeToDelete") {
                        ...
                        if (scan.secondCount >= 0 && scan.secondCount <= 4 && scan.secondCount + scan.line != 49) {
                            theText += "\(scan.hiraganaSet[scan.secondCount + scan.line])"
                            // 選択時の音声
                            ...
                            theText += "\(scan.hiraganaSet[scan.secondCount - 5 + scan.line])"
                            // 選択時の音声
