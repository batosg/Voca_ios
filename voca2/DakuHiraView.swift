                ...
                HStack {
                    ...
                    VStack {
                        ...
                        ZStack {
                            // スキャンボタン
                            Button(action: {
                                dakuHiraScanAction()  // スキャンボタン押下時の処理
                            }) {
                                Text("スキャン")
                    ...
                    Group {
                        VStack {
                            ...
                            ForEach(0..<5) { index in
                                Button(action: {
                                    theText += "\(scan.dakuHiraSet[index])"
                                    // 読み上げ
                                    let utterance = AVSpeechUtterance(string: scan.dakuHiraSet[index])
                                    utterance.voice = AVSpeechSynthesisVoice(language: "ja-JP")
                                    utterance.rate = 0.5
                                    utterance.volume = playvol
                                    let synthesiser = AVSpeechSynthesizer()
                ...
                if (scan.selected == "gagigugego") {
                    ...
                    if (scan.selected == "kirikaeToDelete") {
                        ...
                        if (scan.secondCount >= 0 && scan.secondCount <= 4 && scan.secondCount + scan.line != 39) {
                            theText += "\(scan.dakuHiraSet[scan.secondCount + scan.line])"
                            // 選択時の音声
                            let utterance = AVSpeechUtterance(string: scan.dakuHiraSet[scan.secondCount + scan.line])
                            utterance.voice = AVSpeechSynthesisVoice(language: "ja-JP")
                            utterance.rate = 0.5
                            let synthesiser = AVSpeechSynthesizer()
                            synthesiser.speak(utterance)
                            ...
                            theText += "\(scan.dakuHiraSet[scan.secondCount - 5 + scan.line])"
                            // 選択時の音声
                            let utterance = AVSpeechUtterance(string: scan.dakuHiraSet[scan.secondCount - 5 + scan.line])
                            utterance.voice = AVSpeechSynthesisVoice(language: "ja-JP")
                            utterance.rate = 0.5
                            let synthesiser = AVSpeechSynthesizer()
                            synthesiser.speak(utterance)
